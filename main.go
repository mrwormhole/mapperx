...
func main() {
	...
	err := generateMapperx(sourceTypeName, sourceStructType, sourceTypePackage, targetTypeName, targetStructType, targetTypePackage)
	if err != nil {
		exitWithError(err)
	}
...
func generateMapperx(sourceTypeName string, sourceStructType *types.Struct, sourceTypePackage string, targetTypeName string, targetStructType *types.Struct, targetTypePackage string) error {
	f := NewFile("mapperx")
	f.PackageComment("Code generated by mapperx, PLEASE DO NOT EDIT.")

	functionName := fmt.Sprintf("map%sTo%s", sourceTypeName, targetTypeName)
	if sourceTypeName == targetTypeName {
		return fmt.Errorf("expect 2 different types")
	}

	// form the function signature
	signature := f.Func().Id(functionName).Params(
		Id(camelCase(sourceTypeName)).Op("*").Qual(sourceTypePackage, sourceTypeName),
		Id(camelCase(targetTypeName)).Op("*").Qual(targetTypePackage, targetTypeName),
	)
	...
	for i := 0; i < sourceStructType.NumFields(); i++ {
		...
		for j := 0; j < targetStructType.NumFields(); j++ {
			targetField := targetStructType.Field(j)
			if sourceField.Name() == targetField.Name() && sourceField.Type().Underlying() == targetField.Type().Underlying() {
				fmt.Println("MATCHED")
				fmt.Printf("source: %s , target: %s \n", sourceField.Name(), targetField.Name())

				switch sourceField.Type().(type) {
				case *types.Basic:
					line := Id(camelCase(targetTypeName)).Dot(targetField.Name()).
						Op("=").
						Id(camelCase(sourceTypeName)).Dot(sourceField.Name())
