...
func generateMapperx(sourceTypeName string, sourceStructType *types.Struct, targetTypeName string, targetStructType *types.Struct) error {
	...
	signature := f.Func().Id(functionName).Params(
		Id(camelCase(sourceTypeName)).Op("*").Id(sourceTypeName),
		Id(camelCase(targetTypeName)).Op("*").Id(targetTypeName),
	)
	...
	for i := 0; i < sourceStructType.NumFields(); i++ {
		...
		for j := 0; j < targetStructType.NumFields(); j++ {
			otherField := targetStructType.Field(j)
			if field.Name() == otherField.Name() && field.Type() == otherField.Type() {
				fmt.Println("MATCHED")
				fmt.Printf("source: %s , target: %s \n", field.Name(), otherField.Name())

				switch field.Type().(type) {
				case *types.Basic:
					line := Id(camelCase(sourceTypeName)).Dot(field.Name()).Op("=").Id(camelCase(targetTypeName)).Dot(otherField.Name())
